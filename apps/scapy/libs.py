...
def get_nanjing_weather():
    ...
    today_tem = content_html.findAll("i",limit=1)
    nj_weather=[]
    nj_weather.append([detail_day[0].text,detail_wea[0].text,today_tem[0].text,detail_wins[1].text])
    for i in range(1,7):
        nj_weather.append([detail_day[i].text,detail_wea[i].text,detail_tem_pretty[i-1],detail_wins[2*i+1].text])
        #返回的分别是日期、天气、气温、风力
    return nj_weather

#获取南航校园宣讲就业信息(详情页地址，公司名，宣讲地点，日期)
def get_nuaa_xyxj():
    html = request.urlopen("http://job.nuaa.edu.cn/").read()
    soup = BeautifulSoup(html,'html.parser')
    origin_news = soup.findAll("div",{"class":"","style":""})[1]
    news = origin_news.findAll("ul",{"class":"css-context"})
    xyxj = news[0].findAll("li")
    xyxj_list=[]
    for i in range(len(xyxj)):
        place = xyxj[i].find("div",{"class":"my-companyname pull-left"}).text
        time = xyxj[i].find("span").text
        href = "http://job.nuaa.edu.cn"+xyxj[i].contents[0].attrs["href"]
        xyxj_list.append([href,xyxj[i].contents[0].attrs["title"],place,time])
    return xyxj_list

#获取南航校外招聘就业信息(详情页地址，公司名，宣讲地点，日期)
def get_nuaa_xwzp():
    html = request.urlopen("http://job.nuaa.edu.cn/").read()
    soup = BeautifulSoup(html,'html.parser')
    origin_news = soup.findAll("div",{"class":"","style":""})[1]
    news = origin_news.findAll("ul",{"class":"css-context"})
    xyxj = news[1].findAll("li")
    xyxj_list=[]
    for i in range(len(xyxj)):
        # place = xyxj[i].find("div",{"class":"my-companyname pull-left"}).text
        time = xyxj[i].find("span").text
        href = "http://job.nuaa.edu.cn"+xyxj[i].contents[0].attrs["href"]
        xyxj_list.append([href,xyxj[i].contents[0].attrs["title"],time])
    return xyxj_list

#获取南航招聘会就业信息(详情页地址，公司名，宣讲地点，日期)
def get_nuaa_zph():
    html = request.urlopen("http://job.nuaa.edu.cn/").read()
    soup = BeautifulSoup(html,'html.parser')
    origin_news = soup.findAll("div",{"class":"","style":""})[1]
    news = origin_news.findAll("ul",{"class":"css-context"})
    xyxj = news[2].findAll("li")
    xyxj_list=[]
    for i in range(len(xyxj)):
        place = xyxj[i].find("div",{"class":"my-companyname pull-left"}).text
        time = xyxj[i].find("span").text
