...
import oauth2
import re
from ZhihuiSMB.libs.oauthlib import save_token
import json
...
class UserModel(model.StandCURDModel):
    ...
    def new(self):
        ...
        password = utils.generate_password(self.get_argument("password"),job_no)
    ...
    def login(self,username,password):
        ...
        user = self.coll.find_one({"job_no":username,"password":utils.generate_password(password,username),"enable_flag":1})
        if user is None:
    ...
    def changepsw(self,username,oldpsw,newpsw):
        ...
        user = self.coll.find_one({"job_no":username})
        if not user:
    ...
    def getcontact(self,position):
        if position=="其他":
            user = self.coll.find({"position": {"$in": ["生产", "服务", "其他"]}})
        elif position == "all":
            user=self.coll.find({})
        else:
            user = self.coll.find({"position": position})
        user_contact=[]
        for i in user:
            pc_user = {
                "value":i['job_no'],
